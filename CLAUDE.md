...
- 시장 뉴스: "📰 미국 시장 뉴스" 버튼 → Finnhub general news 헤드라인 20개
  + Claude AI 한국어 브리핑(선택, pf_aikey 재사용). 상단 시세줄에 미국장 상태(프리마켓/정규장/애프터마켓/장 마감) 표시
  + 제목 무료 번역 버튼: MyMemory API (키 불필요, 하루 약 5,000자/기기 한도, 원문은 title 속성에 보존)
- 기기 간 데이터 옮기기: ⚙설정 "내보내기 링크" → 수량·평단가를 #sync= 해시(base64)에 담은 링크 생성,
  다른 기기에서 열면 confirm 후 덮어쓰기. API 키는 링크에 포함하지 않음 (보안)
- 참고: Finnhub 무료 플랜은 시간외(프리/애프터) 가격 미반영 — 사용자 확인 완료, 유료 전환은 보류
